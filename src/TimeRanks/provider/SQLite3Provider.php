...
class SQLite3Provider implements TimeRanksProvider{
	...
	public function isPlayerRegistered(string $name) : bool{
		return $this->db->querySingle("SELECT * FROM timeranks WHERE name = '".\SQLite3::escapeString(strtolower($name))."'") !== null;
	}

	public function registerPlayer(string $name) : void{
		$this->db->exec("INSERT OR IGNORE INTO timeranks (name, minutes) VALUES ('".\SQLite3::escapeString(strtolower($name))."', 0)");
	}

	public function getMinutes(string $name) : int{
		$res = $this->db->querySingle("SELECT minutes FROM timeranks WHERE name = '".\SQLite3::escapeString(strtolower($name))."'", true);
		if($res !== null){
	...
	public function setMinutes(string $name, int $minutes) : void{
		$this->db->exec("UPDATE timeranks SET minutes = '".$minutes."' WHERE name = '".\SQLite3::escapeString(strtolower($name))."'");
	}
