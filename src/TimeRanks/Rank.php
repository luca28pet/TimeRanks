...
class Rank{
    ...
    public function __construct(Main $timeRanks, $name, array $data){
        try{
            ...
            if($pureGroup !== null){
                ...
                throw new \InvalidArgumentException("Rank has not been initialized. PurePerms group ".$data["pureperms_group"]." cannot be found");
            }
            ...
            if(isset($data["name"]) and isset($this->timeRanks->ranks[$data["name"]])){
                unset($this->timeRanks->ranks[$data["name"]]);
            }
        }
    }
    ...
    public function onRankUp($playerName){
        ...
        if($player !== null and $player->isOnline()){
            if(isset($this->message)){
                ...
                $player->sendMessage("Congratulations, you are now rank ".$this->getName());
            }
        }else{
            $player = $this->timeRanks->getServer()->getOfflinePlayer($playerName);
        }
        foreach($this->commands as $cmd){
            $this->timeRanks->getServer()->dispatchCommand(new ConsoleCommandSender(), str_replace("{player}", $player->getName(), $cmd));
        }
